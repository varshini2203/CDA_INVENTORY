...
//
// `branch` and `status` are normalized to the exact values the rest of the
// app compares against — NOT the free-text labels from the spreadsheets:
//   - branch: DroneService.branches is ['CDA Admin', 'CDA Ops']. The more
//     granular original locations ("BRANCH 1 - ADAMBAKKAM", "CDA Ops -
//     Main Facility", "CDA Ops - RPTO", "CDA Ops - Storage Facility") are
//     preserved in `description` instead, and collapsed here to whichever
//     of the two the drone belongs to.
//   - status: Drone.status is only ever 'IN' or 'OUT' elsewhere in the app
//     (see drone_in_out_screen.dart's IN/OUT filter chips, counters, and
//     toggle button). Original labels map as: "In Stock" -> "IN",
//     "Checked Out" -> "OUT", "Retired" -> "IN" (the model has no separate
//     retired state; retirement is still noted in `description`).
// =============================================================================
...
Future<void> seedDrones(FirebaseFirestore firestore) async {
  ...
  final List<Map<String, dynamic>> existingDrones = existingSnapshot.docs
      .map((doc) => doc.data())
      .toList(); // growable: true (default) — required because we .add() below
  ...
  for (final Map<String, dynamic> drone in seedDroneData) {
    ...
    batch.set(newDocRef, {
      ...drone,
      // Drone.fromMap() reads the snake_case key `serial_number`, not the
      // camelCase `serialNumber` used above for dedupe matching. Add the
      // key the app actually reads so serial numbers display correctly.
      ...
      // REQUIRED: DroneService._fetchAllDrones() queries
      // `.orderBy('last_updated', descending: true)`, and Firestore
      // silently omits any document missing the orderBy field from the
      // results entirely. Without this, seeded drones never appear in
      // the app even though the writes succeed.
