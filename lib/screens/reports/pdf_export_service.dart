...
class PdfExportService {
  ...
  static Future<Uint8List> buildNewProductsReport(
      List<NewProduct> rows, DateTime month) async {
    final logoBytes = await _loadLogo();
    ...
    doc.addPage(pw.MultiPage(
      ...
      build: (ctx) => [
        pw.Text(
            '${rows.length} products  —  Stock Value: ${_currency.format(totalValue)}',
            style: pw.TextStyle(color: _muted, fontSize: 10)),
        pw.SizedBox(height: 10),
        _newProductsTable(rows),
      ],
    ));
    return doc.save();
  }
  ...
  static Future<Uint8List> buildStockItemsReport(
      List<StockItem> rows, DateTime asOf) async {
    final logoBytes = await _loadLogo();
    ...
    doc.addPage(pw.MultiPage(
      ...
      footer: (ctx) => _footer(ctx),
      build: (ctx) => [
        pw.Text('${rows.length} items  —  $lowCount low stock',
            style: pw.TextStyle(color: _muted, fontSize: 10)),
        pw.SizedBox(height: 10),
        _stockItemsTable(rows),
      ],
    ));
    return doc.save();
  }
  ...
  static pw.Widget _newProductsTable(List<NewProduct> rows) {
    if (rows.isEmpty) return _emptyNote('No new products in this period.');
    return pw.TableHelper.fromTextArray(
      headers: const [
        'Product', 'Category', 'Vendor', 'Branch', 'Qty', 'Purchase Cost', 'Stock Value', 'Status', 'Date'
      ],
      data: rows
          .map((p) => [
        p.productName,
        ...
        p.branch,
        '${p.quantity}',
        _currency.format(p.purchaseCost),
        _currency.format(p.stockValue),
        p.status,
  ...
  static pw.Widget _stockItemsTable(List<StockItem> rows) {
    if (rows.isEmpty) return _emptyNote('No stock items match these filters.');
    return pw.TableHelper.fromTextArray(
      headers: const ['Product', 'Category', 'Branch', 'Qty', 'Min Stock', 'Unit', 'Location', 'Status'],
      data: rows
          .map((i) => [
        i.productName,
        i.category == 'fixed_asset' ? 'Fixed Asset' : 'Consumable',
        i.branch,
        '${i.quantity}',
        '${i.minStock}',
        i.unit,
