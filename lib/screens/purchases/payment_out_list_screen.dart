...
class PaymentOutListScreen extends StatefulWidget {
  const PaymentOutListScreen({super.key});

  @override
  State<PaymentOutListScreen> createState() => _PaymentOutListScreenState();
}

class _PaymentOutListScreenState extends State<PaymentOutListScreen> {
  List<PaymentOut> _all = [];
  List<PaymentOut> _filtered = [];
  ...
  Future<void> _fetch() async {
    ...
    try {
      final data = await PaymentOutService.getAllPaymentOuts();
      if (!mounted) return;
      setState(() {
        _all = data;
        _isLoading = false;
  ...
  Future<void> _delete(String id) async {
    ...
    if (!confirm) return;
    final result = await PaymentOutService.deletePaymentOut(id);
  ...
  Future<void> _editPayment(PaymentOut p) async {
    final result = await Navigator.push<bool>(
      context,
      MaterialPageRoute(
        settings: const RouteSettings(name: 'Edit Payment Out'),
        builder: (_) => AddPaymentOutScreen(paymentToEdit: p),
      ),
    );
  ...
  void _viewPayment(PaymentOut p) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
                ...
                Container(
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: 'Search vendor, reference...',
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        onPressed: () async {
          final result = await Navigator.push<bool>(
              context, MaterialPageRoute(settings: const RouteSettings(name: 'Add Payment Out'), builder: (_) => const AddPaymentOutScreen()));
          if (result == true) _fetch();
        },
      ),
    );
  ...
  Widget _buildCard(PaymentOut p, int index) {
